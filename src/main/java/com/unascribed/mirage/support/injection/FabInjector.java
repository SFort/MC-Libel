		...
		targetClass.methods.forEach(methodNode -> {
			...
			if (inject != null && mixin != null) {
				...
				injects.add(new ToInject(
						((List<String>) inject.values.get(inject.values.indexOf("method") + 1)),
						((List<String>) inject.values.get(inject.values.indexOf("target") + 1)),
						targetClass.name,
						methodNode.name,
						methodNode.desc,
						methodNode.access,
	...
	public static boolean performInjection(MethodNode methodNode, MethodInsnNode insn, ToInject toInject, String target) {
		...
		if ("Lcom/unascribed/mirage/support/injection/ModifyReturn;".equals(toInject.annotation)) {
			...
			if (optionalReturn) {
				...
				mod.add(new VarInsnNode(Opcodes.ALOAD, max));
				mod.add(new FieldInsnNode(Opcodes.GETFIELD, "com/unascribed/mirage/support/injection/HijackReturn", "object", "Ljava/lang/Object;"));
