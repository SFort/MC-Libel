...
import com.unascribed.libel.support.MixinConfigPlugin;
import com.unascribed.libel.support.OptionalFScript;
...
@Mod(modid = "libel", name = "Libel", version = "3.0")
public class FabricationMod {
	...
	public void init(FMLPostInitializationEvent event) {
		if (isClient) {
			LEVELUP_LONG = new SoundEvent(new ResourceLocation("libel", "levelup_long"));
			OOF = new SoundEvent(new ResourceLocation("libel", "oof"));
			ABSORPTION_HURT = new SoundEvent(new ResourceLocation("libel", "absorption_hurt"));
		}
		for (String str : MixinConfigPlugin.discoverClassesInPackage("com.unascribed.libel.loaders", false)) {
			try {
		...
		for (String s : MixinConfigPlugin.discoverClassesInPackage("com.unascribed.libel.features", false)) {
			try {
	...
	private static final ResourceLocation CONFIG = new ResourceLocation("libel", "config");
