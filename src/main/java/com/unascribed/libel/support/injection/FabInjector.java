package com.unascribed.libel.support.injection;

import com.unascribed.libel.FabLog;
import com.unascribed.libel.support.FabRefMap;
...
public class FabInjector {
	...
	public static void apply(ClassNode targetClass){
		...
		targetClass.methods.forEach(methodNode -> {
			...
			for(AnnotationNode annotationNode : methodNode.visibleAnnotations){
				if ((
						"Lcom/unascribed/libel/support/injection/ModifyReturn;".equals(annotationNode.desc)
						|| "Lcom/unascribed/libel/support/injection/Hijack;".equals(annotationNode.desc)
		...
		injects.forEach(toInject -> redirects.forEach(redirect -> {
			...
			if (toInject.target.contains(redirect.target)) {
	...
	public static boolean performInjection(MethodNode methodNode, MethodInsnNode insn, ToInject toInject, String target) {
		...
		if ("Lcom/unascribed/libel/support/injection/ModifyReturn;".equals(toInject.annotation)) {
			if (--countDesc > 0) {
				...
				for (ListIterator<AbstractInsnNode> it = newVars.iterator(); it.hasNext();) {
					AbstractInsnNode a = it.next();
					if (countDesc-->0) mod.add(a.clone(new HashMap<>()));
				}
				for (ListIterator<AbstractInsnNode> it = oldVars.iterator(); it.hasNext();) {
					AbstractInsnNode a = it.next();
					if (countDesc-->0) mod.add(a.clone(new HashMap<>()));
				}
			...
			return true;
		} else if ("Lcom/unascribed/libel/support/injection/Hijack;".equals(toInject.annotation)) {
			for (int c = 0; c < argTypes.size(); c++) {
			...
			if (optionalReturn) {
				...
				mod.add(new VarInsnNode(Opcodes.ALOAD, max));
				mod.add(new FieldInsnNode(Opcodes.GETFIELD, "com/unascribed/libel/support/injection/HijackReturn", "object", "Ljava/lang/Object;"));
