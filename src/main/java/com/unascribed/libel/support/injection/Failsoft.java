...
public class Failsoft {
	...
	private static boolean doctorStack(StackTraceElement[] stackTrace, StackTraceElement[] newStackTrace) {
		...
		for (int i = 0; i < stackTrace.length; i++) {
			...
			if (ste.getClassName().startsWith("com.unascribed.libel.support.injection.")) {
	...
	public static boolean postInject(InjectionInfo ii, MixinTargetContext mixin, String description, String extraInfo) {
		if (!mixin.getClassName().startsWith("com.unascribed.libel.mixin.")) {
			return true;
		}
		...
		if ((mixin.getOption(Option.DEBUG_INJECTORS) && ii.getInjectedCallbackCount() < expectedCallbackCount)) {
			handleApplyErrorProactively(mixin.getTargetClassInfo().getClassName(), new Exception(
	...
	private static <T extends Throwable> void handleApplyErrorProactively(String targetClassName, T th, IMixinInfo mixin, ErrorAction action) {
		...
		if (action == ErrorAction.ERROR) {
			FabLog.error("Mixin application failed", th);
