...
import com.unascribed.libel.FeaturesFile.FeatureEntry;
import com.unascribed.libel.QDIni.BadValueException;
import com.unascribed.libel.QDIni.IniTransformer;
import com.unascribed.libel.QDIni.SyntaxErrorException;
import com.unascribed.libel.support.ConfigLoader;
import com.unascribed.libel.support.ConfigValue;
import com.unascribed.libel.support.ResolvedConfigValue;
import com.unascribed.libel.support.SpecialEligibility;
...
public class FabConf {
	...
	private static class FeaturesIniTransformer implements IniTransformer {
		...
		public String transformLine(String path, String line) {
			if ((!encounteredNotices && line == null) || (line != null && line.trim().equals(NOTICES_HEADER))) {
				...
				if (badKeys) {
					notices.add("Consider updating this config file by renaming it to libel.ini.old");
				}
				if (notices.isEmpty()) {
	...
	public static RuntimeException devError(String msg) {
		try {
			...
			dummyFrame.setIconImage(Toolkit.getDefaultToolkit().createImage(FabConf.class.getClassLoader().getResource("assets/libel/icon.png")));
			dummyFrame.setSize(1, 1);
			dummyFrame.setLocationRelativeTo(null);
			JOptionPane.showOptionDialog(dummyFrame, msg, "Libel Dev Error",
	...
	public static void worldSet(String configKey, String newValue) {
		...
		set(configKey, newValue, worldPath.resolve("libel").resolve("features.ini"), true);
	}

	public static void set(String configKey, String newValue) {
		set(configKey, newValue, FabricationMod.confPath.toPath().resolve("libel").resolve("features.ini"), false);
	}
	...
	public static void reload() {
		...
		Path dir = FabricationMod.confPath.toPath().resolve("libel");
		try {
			Files.createDirectories(dir);
		} catch (IOException e1) {
			throw new RuntimeException("Failed to create libel config directory", e1);
		}
	...
	public static void worldReload() {
		...
		Path dir = worldPath.resolve("libel");
		try {
			Files.createDirectories(dir);
		} catch (IOException e1) {
			throw new RuntimeException("Failed to create libel world config directory", e1);
		}
		Path configFile = dir.resolve("features.ini");
	...
	private static void checkForAndSaveDefaultsOrUpgrade(Path configFile, String defaultName, IniTransformer transformer) {
		if (!Files.exists(configFile)) {
			Path configFileLegacy = configFile.getParent().getParent().resolve(defaultName.equals("default_features_config.ini") ? "libel.ini" : "libel_"+configFile.getFileName().toString());
